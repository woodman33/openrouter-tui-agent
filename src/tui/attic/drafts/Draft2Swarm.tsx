  ...
  return (
    <Box flexDirection="column" width={width} height={height} borderStyle="round" borderColor={theme.accent}>
      ...
      <Box justifyContent="space-between" paddingX={1} borderStyle="single" borderColor={theme.line}>
        <Box gap={1}>
          <Text color={theme.accent} bold>☷ SWARM COMMANDER</Text>
          <Text color={theme.textMuted}>|</Text>
          <Text color={theme.textSecondary}>Filter:</Text>
          {laneFilters.map((f, idx) => (
            <Text key={f} color={activeLaneFilter === idx ? theme.accent : theme.textMuted} bold={activeLaneFilter === idx}>
              {f}
            </Text>
          ))}
        </Box>
        <Box gap={1}>
          ...
          <Text color={theme.accent}>3/3</Text>
          <Text color={theme.textMuted}>|</Text>
          <Text color={theme.warn}>Gates: 1 Pending</Text>
        </Box>
      </Box>
      ...
      <Box flexGrow={1} flexDirection="row">
        ...
        <Box flexDirection="column" width={22} borderStyle="single" borderColor={theme.line} paddingX={1}>
          <Box marginBottom={1}>
            <Text color={theme.textMuted} bold>── AGENT BUS ──</Text>
          </Box>
          {tabs.map((tab, idx) => {
          ...
          <Box marginTop={1} flexDirection="column" borderStyle="single" borderColor={theme.surfaceRaised} padding={1}>
            <Text color={theme.textMuted}>MUX: tmux 3.4</Text>
            <Text color={theme.textMuted}>ROUTER: round-robin</Text>
            <Text color={theme.accent}>POLICY: balanced</Text>
          </Box>
        </Box>
        ...
        <Box flexDirection="column" width={leftPaneW} borderStyle="single" borderColor={theme.line} paddingX={1}>
              ...
              return (
                ...
                >
                  <Box justifyContent="space-between">
                    <Text color={theme.accent} bold>{task.id} · [{task.agent}]</Text>
                    <Text color={task.status === 'GATE BLOCKED' ? theme.warn : task.status === 'RUNNING' ? theme.accent : theme.accent} bold>
                      {task.status} ({task.progress})
                    </Text>
                  </Box>
                  <Text color={theme.textPrimary}>{task.title}</Text>
                  {isSelected && task.status === 'GATE BLOCKED' && (
                    <Box marginTop={1} borderStyle="single" borderColor={theme.warn} paddingX={1} justifyContent="space-between">
                      <Text color={theme.warn}>⚠️ Risk: Deploying without sealed receipt</Text>
                      <Box gap={1}>
                        <Text color={theme.accent} bold>[A] Approve</Text>
                        <Text color={theme.danger} bold>[K] Deny</Text>
                      </Box>
                    </Box>
                  )}
                </Box>
          ...
          <Box borderStyle="single" borderColor={theme.surfaceRaised} paddingX={1}>
            ...
            <Text color={theme.accent}>[S] Spawn Lane </Text>
            <Text color={theme.accent}>[D] Delegate Task </Text>
            <Text color={theme.accent}>[T] Attach Terminal </Text>
          </Box>
        </Box>
        ...
        <Box flexDirection="column" width={rightPaneW} borderStyle="single" borderColor={theme.line} paddingX={1}>
          <Box justifyContent="space-between" borderStyle="single" borderColor={theme.surfaceRaised} paddingX={1}>
            ...
            <Text color={tailLocked ? theme.accent : theme.warn}>{tailLocked ? '[LOCKED ⇊]' : '[FREE SCROLL]'}</Text>
          </Box>

          <Box marginTop={1} flexDirection="column" flexGrow={1} justifyContent="flex-end" gap={0}>
            {upwardLogs.map((log, i) => (
              <Box key={i} marginY={0}>
                <Text color={theme.textMuted}>{log.seq} </Text>
                <Text color={log.ok ? theme.accent : theme.warn} wrap="truncate-end">
                  {log.txt}
                </Text>
              </Box>
            ))}
          ...
          <Box flexDirection="row" justifyContent="space-between" borderStyle="round" borderColor={theme.line} paddingX={1} marginTop={1}>
            <Text color={theme.textSecondary}>Stdout Rate: <Text color={theme.accent}>14.8 KB/s</Text></Text>
      ...
      <Box flexDirection="column" borderStyle="single" borderColor={theme.line} paddingX={1}>
        <Box justifyContent="space-between">
          <Box gap={1}>
            ...
            <Text color={theme.accent}>[A] Approve Gate</Text>
            <Text color={theme.danger}>[K] Kill Session</Text>
            <Text color={theme.accent}>[Tab] Cycle Filters</Text>
          </Box>
